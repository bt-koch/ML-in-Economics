...
in classification and regression trees, can be utilised [@jarmulska2020]:
...
number of variables considered by the random forest algorithm, i.e. 4.
...
As seen in Table \@ref(tab:avg-pred-accur), the model using random forest outperforms
...
higher influence in the prediction. In line with the results obtained by
...
is fitted, depending on the varying variable to be considered. Certain artificially
created observations could bias the results, especially in the case of correlated
variables, since these observations might be implausible in reality [@jarmulska2020].
...
curves is recognisable when comparing partial dependence against local effects.
When examining these plots for the independent variables used in this model, for
many variables a U-shaped curve can be detected, indicating that both up- and
...
it is still worth to further develop this tools.

Examining the prediction accuracy over time, a drop can be observed during the
sovereign debt crisis (2010-2012). This illustrates that a purely quantitative
model, as the proposed logit lasso and the random forest model, are unable to
directly consider qualitative factors as the reputation or credibility of an
economy [@jarmulska2020]. The strength of fiscal institutions is also a key
factor influencing the probability of fiscal stress episodes, which is not taken
into account in the models shown. Therefore, @jarmulska2020 proposes the
construction of an index which enables to consider these missing qualitative
...
hier noch grafik zu pred accuracy over time

```{r test, fig.cap="lalala"}
plot(x = 1:10, y = 1:10)
```
